...
export const PUT = authMiddleware(async (request, { params }) => {
  try {
    ...
    // In your API route ([id]/route.js), modify the update part:
const updatedSchedule = await prisma.schedule.update({
  where: { id: parseInt(id) },
  data: {
    classroomId: parseInt(classroomId),
    teacherId: teacherId ? parseInt(teacherId) : null,
    startTime: new Date(startTime), // Already in correct local time
    endTime: new Date(endTime),     // Already in correct local time
