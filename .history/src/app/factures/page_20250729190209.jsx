...
import { Button as BootstrapButton, Container, Table, Form, Modal, FormCheck } from 'react-bootstrap';
...
const InvoicesList = () => {
  ...
  const [pricePerSchedule, setPricePerSchedule] = useState(100);
  ...
  const fetchNonInvoicedSchedules = async (teacherId = null) => {
    try {
      const url = teacherId
        ? `/api/schedules/non-invoiced?teacherId=${teacherId}`
        : '/api/schedules/non-invoiced';
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const items = selectedSchedules.map((id) => {
        ...
        return {
          description: schedule?.description || `Horaire ${schedule?.classroom.name} (${new Date(schedule?.startTime).toLocaleString('fr-FR')})`,
          quantity: 1,
