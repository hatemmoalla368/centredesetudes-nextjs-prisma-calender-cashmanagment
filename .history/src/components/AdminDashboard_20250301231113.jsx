...
const AdminDashboard = () => {
  ...
  const columns = useMemo(
    () => [
      {
        accessorKey: "name", // Access nested property classroom.name
        header: "Classroom",
        size: 100,
      },
      {
      ...
      },
      {
        ...
        Cell: ({ cell }) => (
          <div>
            {/* Edit Button */}
            <Button size="small" className="text-primary btn-link edit">
              <Link href={`/admin/schedules/update/${cell.row.original.id}`}>
                <ModeEditIcon />
              </Link>
            </Button>
            {/* Delete Button */}
            <Button
              onClick={() => handleDeleteSchedule(cell.row.original.id)}
              variant="contained"
              color="secondary"
              size="small"
  ...
  return (
    <div className="container">
      ...
      <MaterialReactTable
          key={classrooms.length} // Force re-render when data changes
          columns={columns}
          data={classrooms}
