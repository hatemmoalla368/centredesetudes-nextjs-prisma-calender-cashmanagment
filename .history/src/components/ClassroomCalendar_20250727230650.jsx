...
const ClassroomCalendar = ({ classroomId }) => {
  ...
  // Fetch schedules and teachers (unchanged from original)
  useEffect(() => {
  ...
  const events = schedules.map((schedule) => ({
    ...
    backgroundColor: schedule.recurringWeekly ? '#1a73e8' : '#3174ad',
    borderColor: schedule.recurringWeekly ? '#1a73e8' : '#3174ad',
  }));
  ...
  const handleEventClick = ({ event }) => {
    ...
    setEditingSchedule({
      ...
      recurringWeekly: event.extendedProps.recurringWeekly,
    });
    setShowModal(true);
  };
  ...
  const handleDateSelect = (selectionInfo) => {
    ...
    setEditingSchedule({
      id: null,
      teacherId: '',
      startTime: dateToLocalInput(selectionInfo.start),
      endTime: dateToLocalInput(selectionInfo.end),
      description: '',
      recurringWeekly: false,
    });
    setShowModal(true);
    ...
    <Container className="my-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-4">
        <FullCalendar
          ...
          selectable
          editable
          eventBackgroundColor="#3174ad"
          eventBorderColor="#3174ad"
          eventContent={(eventInfo) => (
            <div className="p-1">
              <strong>{eventInfo.event.title}</strong>
              <br />
              <small>{eventInfo.event.extendedProps.description || 'Aucune description'}</small>
            </div>
          )}
          height="600px"
