...
const InvoicesList = ({ initialInvoice = null }) => {
  ...
  const [invoiceNumber, setInvoiceNumber] = useState('FCT-000001');
  ...
  const generateInvoiceNumber = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Échec de la récupération du dernier numéro de facture');
      const data = await response.json();
      let newNumber = 'FCT-000001';
      if (data.latestInvoice && data.latestInvoice.invoiceNumber) {
        const lastNumber = parseInt(data.latestInvoice.invoiceNumber.split('-')[1], 10);
        if (!isNaN(lastNumber)) {
          newNumber = `FCT-${(lastNumber + 1).toString().padStart(6, '0')}`;
        }
      }
      console.log('Generated invoice number:', newNumber); // Log generated number
      ...
      setInvoiceNumber('FCT-000001'); // Fallback
    }
  };
  ...
  const handleGenerateInvoice = async () => {
    ...
    if (!invoiceNumber.match(/^FCT-\d{6}$/)) {
      toast.error('Numéro de facture invalide');
      return;
    }

    try {
      const quantity = selectedSchedules.length;
      const unitNetPrice = pricePerSchedule - discount;
      const subtotal = quantity * unitNetPrice;
      const taxAmount = subtotal * (taxRate / 100);
      const grandTotal = subtotal + taxAmount + timbreFiscal;
      ...
      const items = selectedSchedules.map((id) => {
        ...
        return {
          description: `Location ${schedule?.classroom.name} (${new Date(schedule?.startTime).toLocaleString('fr-FR')})`,
          quantity: 1,
          price: pricePerSchedule,
          discount: discount,
          total: unitNetPrice,
        };
      });
    ...
    <Container className="my-4">
      ...
      <Modal show={showImportModal} onHide={() => setShowImportModal(false)} size="lg">
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            <Form.Label>Remise (TND)</Form.Label>
            <Form.Control
              type="number"
              name="discount"
              value={discount}
